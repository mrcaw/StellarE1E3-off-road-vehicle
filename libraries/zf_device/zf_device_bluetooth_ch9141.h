...
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Part Start >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
// bluetooth_ch9141_send_byte                                                   // CH9141 模块 发送数据
// bluetooth_ch9141_send_buffer                                                 // CH9141 模块 发送数据块
// bluetooth_ch9141_send_string                                                 // CH9141 模块 发送字符串

// bluetooth_ch9141_read_buffer                                                 // CH9141 模块 读取缓冲

// bluetooth_ch9141_init                                                        // CH9141 模块 初始化
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Part   End <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<

// 此处定义 BLUETOOTH_CH9141 相关状态与数据格式等 这里不允许用户修改
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Part Start >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
typedef enum
{
    BLUETOOTH_CH9141_OPERATION_DONE            =   WIRELESS_INTERFACE_OPERATION_DONE            ,   // 操作完成
    BLUETOOTH_CH9141_ERROR_UNKNOW_ERROR        =   WIRELESS_INTERFACE_ERROR_UNKNOW_ERROR        ,   // 异常错误 这个错误理论上不会被返回

    BLUETOOTH_CH9141_WARNING_MODULE_DISABLE    =   WIRELESS_INTERFACE_WARNING_MODULE_DISABLE    ,   // 模块禁止警告 模块未使能
    BLUETOOTH_CH9141_WARNING_OPERATION_TIMEOUT =   WIRELESS_INTERFACE_WARNING_OPERATION_TIMEOUT ,   // 模块超时警告 通信或者操作超时
    BLUETOOTH_CH9141_WARNING_TRANSFER_LEN_CHANGE                                                ,   // 长度变更警告

    BLUETOOTH_CH9141_ERROR_INTERFACE_OCCUPIED  =   WIRELESS_INTERFACE_ERROR_INTERFACE_OCCUPIED  ,   // 通信接口占用错误 不能重复初始化同一个接口
    BLUETOOTH_CH9141_ERROR_INTERFACE_NOT_INIT  =   WIRELESS_INTERFACE_ERROR_INTERFACE_NOT_INIT  ,   // 接口未初始化错误 不能未初始化就调用接口

    BLUETOOTH_CH9141_ERROR_DATA_BUFFER_NULL    =   WIRELESS_INTERFACE_ERROR_DATA_BUFFER_NULL    ,   // 数据缓冲区错误 不可以传入空指针
}bluetooth_ch9141_operation_state_enum;
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Part   End <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
