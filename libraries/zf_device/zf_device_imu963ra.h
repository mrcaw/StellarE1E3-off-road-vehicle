...
// IMU963RA 磁力计 支持的输出速率为
// IMU_MAG_OUTPUT_RATE_4_DIV        ->  10      Hz
// IMU_MAG_OUTPUT_RATE_BASE         ->  50      Hz
// IMU_MAG_OUTPUT_RATE_2_MUL        ->  100     Hz
// IMU_MAG_OUTPUT_RATE_4_MUL        ->  200     Hz

// IMU963RA 磁力计 支持的量程为
// IMU_MAG_MAG_RANGE_2G             ->  2       Gs
// IMU_MAG_MAG_RANGE_8G             ->  8       Gs

typedef enum
{
    IMU963RA_OPERATION_DONE                 = IMU_INTERFACE_OPERATION_DONE              ,   // 操作完成
    IMU963RA_ERROR_UNKNOW_ERROR             = IMU_INTERFACE_ERROR_UNKNOW_ERROR          ,   // 异常错误 这个错误理论上不会被返回

    IMU963RA_WARNING_MODULE_DISABLE         = IMU_INTERFACE_WARNING_MODULE_DISABLE      ,   // 模块禁止警告 模块未使能
    IMU963RA_WARNING_OPERATION_TIMEOUT      = IMU_INTERFACE_WARNING_OPERATION_TIMEOUT   ,   // 模块超时警告 通信或者操作超时

    IMU963RA_ERROR_INTERFACE_OCCUPIED       = IMU_INTERFACE_ERROR_INTERFACE_OCCUPIED    ,   // 通信接口占用错误 不能重复初始化同一个接口
    IMU963RA_ERROR_INTERFACE_NOT_INIT       = IMU_INTERFACE_ERROR_INTERFACE_NOT_INIT    ,   // 接口未初始化错误 不能未初始化就调用接口
    IMU963RA_ERROR_INTERFACE_TYPE_UNKNOW    = IMU_INTERFACE_ERROR_INTERFACE_TYPE_UNKNOW ,   // 接口错误

    IMU963RA_ERROR_DATA_BUFFER_NULL         = IMU_INTERFACE_ERROR_DATA_BUFFER_NULL      ,   // 数据缓冲区错误 不可以传入空指针

    IMU963RA_ERROR_ID_CHECK_ERROR                                                       ,   // 模块 ID 匹配异常
    IMU963RA_ERROR_SELF_CHECK_ERROR                                                     ,   // 模块自检错误

    IMU963RA_ERROR_SET_CONFIG_ERROR                                                     ,   // 模块配置错误
    IMU963RA_ERROR_SET_ACC_OUTPUT_RATE_ERROR                                            ,   // 加速度计 输出速率 设置错误
    IMU963RA_ERROR_SET_ACC_RANGE_ERROR                                                  ,   // 加速度计 量程 设置错误
    IMU963RA_ERROR_SET_GYRO_OUTPUT_RATE_ERROR                                           ,   // 陀螺仪 输出速率 设置错误
    IMU963RA_ERROR_SET_GYRO_RANGE_ERROR                                                 ,   // 陀螺仪 量程 设置错误
    IMU963RA_ERROR_SET_MAG_OUTPUT_RATE_ERROR                                            ,   // 磁力计 输出速率 设置错误
    IMU963RA_ERROR_SET_MAG_RANGE_ERROR                                                  ,   // 磁力计 量程 设置错误

    IMU963RA_ERROR_MAG_WRITE_RIGESTER_ERROR                                             ,   // 磁力计 写寄存器错误
    IMU963RA_ERROR_MAG_ID_CHECK_ERROR                                                   ,   // 磁力计 ID 匹配异常
    IMU963RA_ERROR_MAG_SET_CONFIG_ERROR                                                 ,   // 磁力计 配置错误
}imu963ra_operation_state_enum;
