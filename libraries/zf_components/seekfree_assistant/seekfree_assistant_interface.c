...
// 使用示例
//-------------------------------------------------------------------------------------------------------------------
ZF_WEAK uint8 seekfree_assistant_transfer (const uint8 *buff, uint32 *length)
{
    (void)buff;
    (void)length;
...
// 使用示例
//-------------------------------------------------------------------------------------------------------------------
ZF_WEAK uint8 seekfree_assistant_receive (uint8 *buff, uint32 *length)
{
    (void)buff;
    (void)length;
...
ZF_WEAK void seekfree_assistant_interface_init (seekfree_assistant_transfer_device_enum transfer_device)
{
    switch(transfer_device)
    {
        ...
        {
            seekfree_assistant_transfer_callback = debug_uart_write_buffer;
            seekfree_assistant_receive_callback = debug_uart_read_buffer;
        }break;

        case SEEKFREE_ASSISTANT_BLUETOOTH_CH9141:
        {
