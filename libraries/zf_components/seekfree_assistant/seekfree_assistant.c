...
extern uint8 seekfree_assistant_transfer (const uint8 *buff, uint32 *length);
extern uint8 seekfree_assistant_receive (uint8 *buff, uint32 *length);
...
static zf_fifo_obj_struct   seekfree_assistant_fifo =                                                                       // FIFO结构体
{
    .base_ptr   = ((uint32)seekfree_assistant_buffer),
    .tail_ptr   = ((uint32)seekfree_assistant_buffer) + SEEKFREE_ASSISTANT_BUFFER_SIZE,
    .write_ptr  = ((uint32)seekfree_assistant_buffer),
    .read_ptr   = ((uint32)seekfree_assistant_buffer),
    .step       = FIFO_DATA_8BIT,
    .state      = 0x01,
};
#endif
...
void seekfree_assistant_camera_data_send (seekfree_assistant_image_type_enum camera_type, void *image_addr, uint8 boundary_num, uint16 width, uint16 height)
{
    ...
    uint32 temp_lenght = sizeof(seekfree_assistant_camera_struct);
    seekfree_assistant_transfer_callback((const uint8 *)&seekfree_assistant_camera_data, &temp_lenght);
    ...
    if(NULL != image_addr)
    {
        temp_lenght = image_size;
        seekfree_assistant_transfer_callback(image_addr, &temp_lenght);
    }
}
...
void seekfree_assistant_camera_dot_send (seekfree_assistant_camera_buffer_struct *buffer)
{
    ...
    uint32 temp_lenght = sizeof(seekfree_assistant_camera_dot_struct);
    seekfree_assistant_transfer_callback((const uint8 *)&seekfree_assistant_camera_dot_data, &temp_lenght);
    ...
    for(i=0; i < SEEKFREE_ASSISTANT_CAMERA_MAX_BOUNDARY; i++)
    {
        ...
        {
            temp_lenght = dot_bytes;
            seekfree_assistant_transfer_callback((const uint8 *)buffer->boundary_x[i], &temp_lenght);
        }
        ...
        if(NULL != buffer->boundary_y[i])
        {
            ...
            temp_lenght = dot_bytes;
            seekfree_assistant_transfer_callback((const uint8 *)buffer->boundary_y[i], &temp_lenght);
        }
    }
}
...
void seekfree_assistant_oscilloscope_send (seekfree_assistant_oscilloscope_struct *seekfree_assistant_oscilloscope)
{
    ...
    uint32 temp_lenght = packet_size;
    seekfree_assistant_transfer_callback((const uint8 *)seekfree_assistant_oscilloscope, &temp_lenght);
}
...
void seekfree_assistant_data_analysis (void)
{
    ...
    read_length = SEEKFREE_ASSISTANT_BUFFER_SIZE;
    seekfree_assistant_receive_callback((uint8 *)temp_buffer, &read_length);
    if(read_length)
    {
        // 将读取到的数据写入FIFO
        zf_fifo_write_buffer(&seekfree_assistant_fifo, temp_buffer, &read_length);
    }

    while(sizeof(seekfree_assistant_parameter_struct) <= zf_fifo_used(&seekfree_assistant_fifo))
    {
        read_length = sizeof(seekfree_assistant_parameter_struct);
        zf_fifo_read_buffer(&seekfree_assistant_fifo, (uint8 *)temp_buffer, &read_length, FIFO_READ_WITHOUT_CLEAN);
        ...
        // 丢弃无需使用的数据
        zf_fifo_read_buffer(&seekfree_assistant_fifo, (uint8 *)temp_buffer, &read_length, FIFO_READ_WITH_CLEAN);
    }
}
#endif
